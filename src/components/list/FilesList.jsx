...
class FilesList extends Component {
  ...
  render() {
    ...
    const selectingSong = this.selectSong;
    const list = player.songsList
      ? Object.keys(player.songsList).map(function(key, index) {
          let currentSong =
            'list-group-item waves-effect waves-light font-weight-bold darken-4 grey ';
          currentSong +=
            player.song.title === player.songsList[key].title
              ? 'text-success'
              : 'text-light';
          return (
            <div
              key={index}
              onClick={() => selectingSong(player.songsList[key])}
              className={currentSong}
            >
              {player.songsList[key].title}
              <p className="font-weight-lighter author-name">
                {player.songsList[key].narrator}
              </p>
            </div>
          );
        })
